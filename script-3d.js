...
import { BloomEffect, ChromaticAberrationEffect, EffectComposer, EffectPass, RenderPass } from 'https://esm.sh/postprocessing@6.33.3';
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const uniforms = {
      ...
      uLinesColor: { value: new THREE.Color(props.linesColor) },
      uScanColor: { value: new THREE.Color(props.scanColor) },
  ...
  if (props.scanOnClick) document.addEventListener('click', onClick);
...
function smoothDampVec2(current, target, currentVelocity, smoothTime, maxSpeed, deltaTime) {
  ...
  let target_new = current.clone().sub(change);
  const temp = currentVelocity.clone().addScaledVector(change, omega).multiplyScalar(deltaTime);
  currentVelocity.sub(temp.clone().multiplyScalar(omega));
  currentVelocity.multiplyScalar(exp);

  out.copy(target_new.clone().add(change.add(temp).multiplyScalar(exp)));
...
function smoothDampFloat(current, target, velRef, smoothTime, maxSpeed, deltaTime) {
  ...
  let target_new = current - change;
  const temp = (velRef.v + omega * change) * deltaTime;
  velRef.v = (velRef.v - omega * temp) * exp;

  let out = target_new + (change + temp) * exp;
